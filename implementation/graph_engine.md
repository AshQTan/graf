...
GraphCanvas (React)
  └─ Stage (Konva)
       ...
       ├─ PathLayer             ← lines connecting sequential points
       └─ PointLayer            ← plot points (topmost, interactive)
```
...
#### Line Renderer
- Horizontal time axis (X), vertical value axis (Y).
- **X position is derived from the point's `timestamp` at render time** — it is NOT stored as a coordinate. The renderer computes `x_pixel` by mapping the timestamp into the visible date range. This means a line graph point's stored coordinates are simply `{"y": 0.6}` — no `x` key.
- Y-axis uses the graph's defined labels/range. `y: [0, 1]` normalized.
- The renderer auto-scales the time axis to fit the date range of existing points, with padding.
- Points are connected chronologically by the `PathLayer`.
- **Implication for `toNormalized()`**: when a user taps to plot on a line graph, the tap's X pixel position is converted to a timestamp (via the time-axis scale), and the Y pixel is normalized to `[0, 1]`. The resulting point stores `{ coordinates: {"y": 0.6}, timestamp: <derived> }`.
...
**All point coordinates are stored as normalized values in `[0, 1]`**, regardless of what the user sees on their axis labels.
...
### Tap-to-Plot (Quadrant, Line, Timeline)
1. User taps the canvas.
2. Konva fires a `tap`/`click` event on the `Stage` (not on an existing point).
3. The interaction handler converts the pixel position to normalized coordinates via the active renderer's `toNormalized()`.
4. If grid snap is on, coordinates are snapped.
5. A new `Point` object is created (with `timestamp: now`, coordinates, and source: `manual`).
6. The `PointInspector` bottom sheet opens, pre-filled with the new point. The user can edit the note/tags/timestamp or just confirm.
7. On save, the point is persisted and the canvas re-renders.
...
- On drag end, the new pixel position is converted back to normalized coordinates and the point is updated.
